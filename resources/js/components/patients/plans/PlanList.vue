<template>
    <div class="tab-wrapper">
        <div class="container with-pb-2">
            <div class="with-p-4 bg-light rounded with-shadow">
                ...
                <div class="component-wrapper-body mt-2 text-dark">
                    ...
                    <div v-else v-for="(p, planIndex) in this.plan" :key="p.id"
                         class="bg-white rounded with-shadow p-4 mb-2">
                        <div class="d-flex justify-content-center align-items-center">
                            ...
                            <button type="button" class="btn btn-link mr-2"
                                    v-on:click="() => { showPlanListStatsPage(p.id, p.dayOfWeek, p.date)}">
                                Consultar Informação Nutricional
                            </button>
                            <button v-show="!readonly" type="button" class="btn btn-primary"
                                    @click="() => { addMealType(p.id, p.dayOfWeek, p.date, planIndex) }">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        ...
                        <div v-for="(mealType, mealTypeIndex) in p.mealTypes" :key="mealType.id"
                             class="bg-light rounded with-shadow p-2 mb-2 d-flex align-items-center">
                            <img
                                :src="`https://nutriclock.s3-eu-west-1.amazonaws.com/mealPlans/thumb_${mealType.photoUrl}`"
                                alt=""
                                class="avatar mr-2 rounded-circle pointer"
                                @error="setAltImage"
                                style="width: 60px"
                            ...
                            <div class="flex-grow-1">
                                <div class="text-primary font-weight-bold" v-if="mealType.confirmed"> Refeicao efetuada pelas {{mealType.confirmedHours}} horas</div>
                                <div class="d-flex align-items-center flex-wrap">
                                    ...
                                    <div :class="`text-secondary ${!mealType.confirmed && 'mr-4'}`">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                             fill="currentColor"
                                    ...
                                    <div v-show="!readonly && !mealType.confirmed">
                                    <span
                                        @click="() => { openAddIngredientsModal(mealTypeIndex, mealType.id, planIndex) }">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                             fill="currentColor" class="bi bi-plus-circle-fill text-info mr-2"
                                             viewBox="0 0 16 16">
                                          <path
                                ...
                                <div v-for="(i, index) in mealType.ingredients" :key="i.id">
                                    <div class="d-flex flex-wrap align-items-center text-secondary mb-1">
                                        ...
                                        <div class="mr-2" style="font-size: 12px; min-width: 110px"> {{ i.quantity }}
                                            {{ i.unit }}
                                        </div>
                                        <div :class="`text-secondary ${!mealType.confirmed && 'mr-2'}`"
                                             style="font-size: 12px; font-weight: 900; min-width: 60px">{{ i.grams }}
                                            gr.
                                        </div>
                                        <div v-show="!readonly && !mealType.confirmed"
                                             @click="() => { deleteIngredient(i.id, index, mealTypeIndex, planIndex) }">
        ...
        <AddIngredientModal @close="closeModal" v-show="showAddIngredientModal" @save="saveIngredientsToMeal"
